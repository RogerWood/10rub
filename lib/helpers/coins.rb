...
module Nanoc3::Helpers
   ...
   # модуль реализует работу с элементами "монета". элемент воспринимается,
   # монетой если поле "layout" имеет значение "coin" в файле *.yaml.

   module Coins
      
      # возвращает неотсортированный массив элементов "монета". определение
      # элемента "монета" производится по полю, описанному в "шапке".
      ...
      # возвращает отсортированный по убыванию даты покупки массив элементов
      # "монета". дата покупки определяется полем "date_of_purchase" в файле *.yaml.
      ...
      # возвращает неотсортированный массив элементов "монета", принадлежащий 
      # элементу "серия". определение элемента "монета" производится по полю, 
      # описанному в "шапке".
      ...
      # возвращает отсортированный по убыванию даты покупки массив элементов 
      # "монета", принадлежащих элементу "серия". дата покупки определяется 
      # полем "date_of_purchase" в файле *.yaml.
      ...
      # возвращает соответствующее склонение для количества монет.
